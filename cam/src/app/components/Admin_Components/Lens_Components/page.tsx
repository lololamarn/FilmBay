...
const AdminPage: React.FC<AdminPageProps> = () => {
  ...
  const handleDeleteLens = async (Lens_ID: string | undefined) => {
    if (Lens_ID) {
      try {
        const response = await fetch(`http://localhost:8080/api/delete-lens/${Lens_ID}`, {
          method: 'DELETE',
          headers: {
        ...
        if (response.ok) {
          console.log('lente eliminada exitosamente');
          window.location.reload();
        } else {
          const data = await response.json();
          console.error('Error al eliminar lente:', data.error || 'Error desconocido');
        }
      } catch (error) {
        console.error('Error de red al eliminar lente:', error);
      }
    } else {
      ...
      {showAddLens && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
          <div className="bg-white p-6 rounded-lg">
            <AddLens handleAdd={() => setShowAddLens(false)} onClose={() => setShowAddLens(false)} /> {/* Cambiar a AddLens */}
          </div>
        </div>
      )}
      ...
      <div className="lg:w-full lg:ml-64 px-6 py-8">
        ...
        <div className="lg:flex gap-4 items-stretch">
          <div className="bg-gray-500 md:p-2 p-6 rounded-lg border border-yellow-400 ">
            <h1 className='text-xl text-center mt-2 mr-2 ml-2'>Información del lente Seleccionado</h1>
            {selectedIndex !== null && (
              <div className="flex justify-center items-center space-x-5 h-full">
                <div className="m-4">
                  <div className="bg-gray-800 p-2 rounded mb-2">
                    <p className="text-2xl font-semibold text-center text-white mb-2">{searchResults[selectedIndex].Lens_Name}</p>
                  </div>
                  ...
                  <div className="mt-4 flex justify-center ">
                    <EditComponent
                      ...
                      Lens_ID={searchResults[selectedIndex]?.Lens_ID} 
                    />
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {searchResults.slice(startIndex, startIndex + 3).map((result, index) => (
                <div key={index} className={`bg-gray-200 p-4 rounded-md cursor-pointer ${selectedIndex === startIndex + index ? 'bg-blue-200' : ''}`} onClick={() => handleSelectResult(startIndex + index)}>
                  <div className="mb-4">
                    <p className='text-black font-semibold'>Nombre del lente:</p>
                    <p className='text-black'>{result.Lens_Name}</p>
                  </div>
                  <div className="mb-4">
                    <p className='text-black font-semibold'>ID lente:</p>
                    <p className='text-black'>{result.Lens_ID}</p>
                  </div>
                  <div className="mb-4">
