...
const SearchBar: React.FC<SearchBarProps> = ({ onSearch }) => {
    ...
    const handleSearch = async () => {
        try {
            const response = await fetch(`http://localhost:8080/api/search-lens?searchTerm=${searchTerm}`, { // Cambiar a search-lens
                method: 'GET',
                headers: {
            ...
            if (response.ok) {
                ...
                throw new Error('Error en la búsqueda de lentes');
            }
        } catch (error) {
